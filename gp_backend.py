...
def reconstruction_grid(x_len, y_len):
    ...
    z = np.linspace(0., 1., y_len)
    # print(x_len, len(R), y_len, len(z))
    # exit(0)
    pixels = []
...
class GaussianProcessEuclidean(GaussianProcess):
    def __init__(self, geo_matrix, prior_mean, grid, #boundary_matrix,
        ...
        self.num_classes = len(self.sigma_fs)*len(self.sigma_xs)*len(self.sigma_errs)
        print('will loop over:', self.num_classes, 'classes.')
        
        pr_covs = []
        print('computing co-variance matrices for gaussian process regression...')
    ...
    def calc_distances(self, grid):
        print('computing point-wise distance in reconstruction grid...')
        distance = []
        k = 0
        for p1 in grid:
            for p2 in grid:
    ...
    def marginalize_and_calculate_hps(self, sample_id, measurement, inf_err_mask, verbose=False):
        ...
        start = time.time()
        if verbose:
            print('sigma_f    ', 'sigma_x    ','sigma_err   ','mll    ','   class')
            ...
            for i, sigma_f in enumerate(self.sigma_fs):
                for j, sigma_x in enumerate(self.sigma_xs):
                    ...
                    try:
                        ...
                        sigma_d_inv = np.linalg.lstsq(sigma_d, np.identity(len(filtered_measurement)))[0]
                    # print(filtered_measurement.shape, sigma_d.shape, fL.shape, sigma_d_inv.shape)
                    # exit(0)
                    ...
                    if mll > bad_mll:
                        ...
                        best_sigma_x = np.zeros(len(self.sigma_xs))
                        best_sigma_err = np.zeros(len(self.sigma_errs))
                        best_sigma_grid = np.zeros(self.num_classes).reshape(len(self.sigma_errs), len(self.sigma_fs), len(self.sigma_xs))
                        best_sigma_f[i] = 1
                        best_sigma_x[j] = 1
                        best_sigma_err[h] = 1
                        ...
                        # exit(0)
                        if verbose:
                            print("%.2f     %6.3f   %9.3f  %15.4f %6d " % (sigma_f, sigma_x, sigma_err, mll, int(single_multiclass[0])))
        end = time.time()
        # print(end - start)
        t_delta = end - start
        return best_post_mean, best_post_cov, best_sigma_d, bad_mll, [best_sigma_err, best_sigma_f, best_sigma_x], best_sigma_grid, single_multiclass,t_delta   # best_sigma_err_ind, best_sigma_f_ind, best_sigma_x_ind
...
class GaussianProcessVectorial(GaussianProcess):
    def __init__(self, geo_matrix, prior_mean, grid, #boundary_matrix,
        ...
        self.num_classes = len(self.sigma_fs)*len(self.sigma_l1s)*len(self.sigma_errs)*len(self.sigma_l2s)
        print('will loop over:', self.num_classes, 'classes.')
        # exit()
        self.distances = self.calc_distances(self.grid)
        pr_covs = []
    ...
    def marginalize_and_calculate_hps(self, sample_id, measurement, inf_err_mask, verbose=False):
                ...
                for j, sigma_l1 in enumerate(self.sigma_l1s):
                    for k, sigma_l2 in enumerate(self.sigma_l2s):
                        ...
                        if mll > bad_mll:
                            ...
                            best_sigma_err = np.zeros(len(self.sigma_errs))
                            best_sigma_grid = np.zeros((self.num_classes)).reshape(len(self.sigma_errs),
                                                                                    len(self.sigma_fs),
                                                                                    len(self.sigma_l1s),
                                                                                    len(self.sigma_l2s))
                            best_sigma_f[i] = 1
        ...
        # print(end - start)
        t_delta = end - start
        return best_post_mean, best_post_cov, best_sigma_d, bad_mll, [best_sigma_err, best_sigma_f, best_sigma_l1, best_sigma_l2],  best_sigma_grid, single_multiclass,t_delta 
